...
/*int main()
{
    ...
    song* Songptrs[MAX_SONG];
    user* userptrs[MAX_USER];
    
    int numsongs=0,numusers=0;

    if(strcmp(ins,"make_song")==0)
    {
        char name[20],artist[20];
        float dur;
        scanf("%s %s %f",name,artist,dur);

        Songptrs[numsongs++]=makesong(name,artist,dur);
    }

    else if(strcmp(ins,"create_user")==0)
    {
        char username[100];
        scanf("%s",username);

        userptrs[numusers++]=createUser(username);
    }

    else if(strcmp(ins,"ADD_SONG_QUEUE")==0)
    {
        char username[100],songname[100];
        scanf("%s %s",username,songname);

        user* requser=NULL;
        for(int i=0;i<numusers;i++)
        {
            if(strcmp(userptrs[i]->name,username)==0)
            {
                requser=userptrs[i];
                break;
            }
        ...
        else
        {
            ...
            {
                if(strcmp(Songptrs[i]->name,songname)==0)
                {
                    reqsong=Songptrs[i];
                    break;
                }
            ...
            {
                int success=addSongToQueueUser(requser,reqsong);
                if(success==0)
    ...
    else if(strcmp(ins,"REMOVE_SONG_QUEUE")==0)
    {
